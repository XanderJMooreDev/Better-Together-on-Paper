...
standSprite = spr_p4_stand;
preJumpSprite = spr_p4_pre_jump;
jumpSprite = spr_p4_jump;
crouchSprite = spr_p4_crouch;

upCommand = vk_numpad5;
leftCommand = vk_numpad1;
rightCommand = vk_numpad3;
downCommand = vk_numpad2;
bombCommand = vk_numpad6;


terrain_tiles = layer_tilemap_get_id("Collision");

hat = instance_create_layer(x, y, "Hats", obj_teamhat);
hat.player = obj_platformer_player4;
...
hat.sprite = ds_list_find_value(obj_game_manager.playerHats, obj_game_manager.playerHatIDs[4]);

check_collisions = function(changeX, changeY) {
	if place_meeting(x + changeX, y + changeY, obj_team_platform) {
		if x < 1000 {
			obj_game_manager.player4Team = 1;
			hat.image_blend = obj_game_manager.teamColor1;
		}
		else {
			obj_game_manager.player4Team = 2;
...
throw_bomb = function() {
	...
	bomb.friendly = obj_game_manager.player4Team;
